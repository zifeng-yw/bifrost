	...
	const onSubmit = async (data: WebhookFormData) => {
		...
		for (const [name, value] of Object.entries(headers)) {
			...
			if (!value?.value?.trim() && !value?.ref?.trim()) {
				toast.error(`Header "${name}" must have a value`);
				return;
			}
				...
				<form onSubmit={handleSubmit(onSubmit)} className="flex min-h-0 flex-1 flex-col">
					<div className="flex-1 space-y-4 overflow-y-auto px-8">
						...
						<Accordion type="single" collapsible className="w-full">
							<AccordionItem value="delivery-tuning" className="border-b-0">
								<AccordionTrigger className="py-0" data-testid="webhook-tuning-trigger">
									Delivery Tuning
								</AccordionTrigger>
								<AccordionContent className="space-y-4 pt-4 pb-1">
										...
										return (
											<div key={field.key} className="flex items-center justify-between gap-4 rounded-lg border px-4 py-2">
												...
												<Input
													...
													value={fieldValue ?? ""}
													onChange={(e) =>
														setValue(field.key, e.target.value === "" ? undefined : parseInt(e.target.value), {
															shouldValidate: true,
															shouldDirty: true,
														})
													}
