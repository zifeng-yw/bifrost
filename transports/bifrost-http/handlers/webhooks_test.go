...
func newWebhookTestHandler(t *testing.T) (*WebhookHandler, *lib.Config) {
	...
	return NewWebhookHandler(config, dispatcher), config
}
...
func TestWebhookHandlerTestDelivery(t *testing.T) {
	...
	// The per-endpoint cooldown rejects an immediate second fire and reports
	...
	assert.Equal(t, fasthttp.StatusTooManyRequests, repeatCtx.Response.StatusCode())
	retryAfter, ok := decodeJSONResponse(t, repeatCtx)["retry_after_seconds"].(float64)
	require.True(t, ok, "429 must carry retry_after_seconds")
	assert.Greater(t, retryAfter, float64(0))
	assert.LessOrEqual(t, retryAfter, float64(30))
...
func TestWebhookHandlerStoreUnavailable(t *testing.T) {
	handler := NewWebhookHandler(&lib.Config{}, nil)
	ctx := newWebhookRequestCtx("", nil)
	handler.listWebhookEndpoints(ctx)
